...
Este repositório contém o código-fonte do Trabalho A3: uma calculadora científica de números complexos em **Java**, capaz de analisar e executar expressões matemáticas, seguindo estritamente as regras de compilação e estruturas de dados (AST).

## 📑 Índice

1. [Sobre o Projeto](#-calculadora-científica-de-números-complexos-a3)
2. [Integrantes](#-integrantes)
3. [Tecnologias Utilizadas](#-tecnologias-utilizadas)
4. [Como Executar (Docker)](#-como-executar-via-docker)
5. [Casos de Uso (Exemplos)](#-casos-de-uso-showcase)
6. [Arquitetura: Números Complexos](#-arquitetura-do-código)
    - [Matemática: Potência e Raiz](#-explicação-lógica-e-matemática-power-e-raiz-nthroot)
7. [Fase 1: Tokenização (Lexer)](#-o-processo-de-tokenização)
8. [Fase 2: Estrutura da Árvore (AST)](#-a-estrutura-da-árvore-ast)
9. [Fase 3: Análise Sintática (Parser)](#-análise-sintática---parserjava)
10. [Fase 4: Execução e Interface (Evaluator)](#-evaluatorjava-e-a-interface-do-usuário)
11. [Interface de Linha de Comando (CLI)](#-interface-de-linha-de-comando-cli)
...
### `ComplexNumber.java`
  ...
  * **Tratamento de Erros:** O método `.divide()` detecta ativamente a divisão por zero (denominador \< 1e-9) e lança uma `ArithmeticException` customizada.
...
Este processo de Tokenização introduz três novos componentes no projeto.
...
Representa a operação matemática de radiciação (Raiz N-ésima). Diferente de uma operação binária comum, a raiz possui um parâmetro fixo inteiro (o grau da raiz).
  ...
  * **Por que um nó separado?** Isso garante a **tipagem forte** do grau da raiz (exigindo que seja um `int`) e facilita a validação da Regra 5 (evitando graus inválidos como zero ou números complexos no índice da raiz).
...
#### 2\. `expression()` (Soma e Subtração)
  ...
  * **Lógica:**
    ...
    2.  Entra em um loop `while`: enquanto encontrar tokens `+` ou `-`, ele consome o operador, chama `term()` novamente para o lado direito e cria um novo `BinaryOperationNode` combinando os dois.
...
#### 4\. `power()` (Potenciação)
  ...
  * **Retorno:** Um nó `BinaryOperationNode` do tipo POWER ou o resultado de `unary()`.

#### 5\. `unary()` (Funções e Raízes)

  * **Precedência:** Altíssima.
  * **Lógica:** Verifica se o token atual é uma palavra-chave de função.
      * **`conj`:** Consome obrigatoriamente `(` + expressão + `)` e retorna um `UnaryOperationNode`.
...
2.  **Nós de Operação (Recursão):**
      ...
      * `BinaryOperationNode` (ex: Soma, Multiplicação, etc): Primeiro resolve o lado **Esquerdo**, depois o **Direito**, e só então aplica a operação matemática.
